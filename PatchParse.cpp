...
*/
	...
	FILE* oh = fopen("patchlist_php", "w");

	dword PatchVersion;
	dword FileCount;
	fread(&PatchVersion, 4, 1, fh);
	fread(&FileCount, 4, 1, fh);
	...
	for (int i = 0; i < FileCount; i++) {
		...
		fread(&CRC, 4, 1, fh);
		fread(&NameLen, 2, 1, fh);
		Name = (char*)calloc(1, NameLen + 1);
		fread(Name, NameLen, 1, fh);
		fread(&VfsLen, 2, 1, fh);
		Vfs = (char*)calloc(1, VfsLen + 1);
		fseek(fh, VfsLen, SEEK_CUR);

		fprintf(oh, "%s\tpatch0.%lu\t0_%lu_%lu.zip\t%lu\n", Name, PatchVersion, Version, MakeStrHash(Name), CRC);
